...
本文件是 `chrome_client` 在 `main` 分支上的执行顺序。它描述
...
`mn_request_resume_read` 加 `on_body` 返回值；WebSocket 侧则通过
`HasPendingDataFrames()` 加 `ResumeReading()` 实现消费者驱动 pause/resume
（未消费帧让 channel 暂停读取，消费完再恢复）。
