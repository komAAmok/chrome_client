...
# Cross-build only. Runtime tests require Windows on ARM or an ARM64 runner.
...
OUT_DIR=${OUT_DIR:-$CHROMIUM_SRC/out/MiniCronet-win-arm64}
JOBS=${JOBS:-2}
GN=$CHROMIUM_SRC/buildtools/linux64/gn
NINJA=$CHROMIUM_SRC/third_party/ninja/ninja
XWIN_ROOT=${XWIN_ROOT:-$(resolve_chromium_toolchain)/xwin-winsysroot}
...
printf 'Windows ARM64 cross build and audit passed: %s\n' "$OUT_DIR"
